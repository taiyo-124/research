...
void setup() {
  ...
  DEBUG_STREAM.println("DEBUG Start");
  ...
  if (SAVE_SD) {
    ...
    #if DEBUG
      DEBUG_STREAM.println("SDCard Initializd");
    #endif
  }
  ...
  if (GET_GPS) {
    ...
    #if DEBUG
      DEBUG_STREAM.println("GPS Initialized");
    #endif
  }
  ...
  #if DEBUG
    DEBUG_STREAM.println("Setup Finished");
...
void loop() {
  ...
  if (SAVE_SD) {
    bool tf = writeSD(filename, outputBuf);
  }
